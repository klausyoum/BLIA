...
# Query to show fixed method information
SELECT A.BUG_ID, B.MTH_ID, B.MTH_NAME, B.RET_TYPE FROM BUG_FIX_MTH_INFO A, MTH_INFO B WHERE A.FIXED_MTH_ID = B.MTH_ID ORDER BY A.BUG_ID

# Query to represent commit log score of method
SELECT A.BUG_ID, A.MTH_ID, A.COMM_SCORE, A.BLIA_MTH_SCORE, B.SF_VER_ID, B.MTH_NAME, B.RET_TYPE, B.PARAMS FROM INT_MTH_ANALYSIS A, MTH_INFO B WHERE A.MTH_ID = B.MTH_ID
